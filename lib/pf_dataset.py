...
class PFPascalDataset(Dataset):
    ...
    def singleToThree(self, a):  # to correct the error for 3-color-gray matching image
    ...
    def get_image(self,img_name_list,idx):
        ...
        if len(image.shape) == 2:
            image = self.singleToThree(image)
        # get image size
    ...
    def get_points(self,point_coords_list,idx):
        ...
        Xpad = -np.ones(mem); Xpad[:len(X)]=X
        Ypad = -np.ones(mem); Ypad[:len(X)]=Y
        point_coords = np.concatenate((Xpad.reshape(1,mem),Ypad.reshape(1,mem)),axis=0)
