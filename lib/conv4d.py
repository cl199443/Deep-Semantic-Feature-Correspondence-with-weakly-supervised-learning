...
def conv4d(data,filters,bias=None,permute_filters=True,use_half=False):
    ...
    # data: [batch_size, 1, 16, 16, 16, 16]  # filters:
    data=data.permute(2,0,1,3,4,5).contiguous()  # [16, batch_size, 1, 16, 16, 16] # permute to avoid making contiguous inside loop
        
    # Same permutation is done with filters, unless already provided with permutation
    if permute_filters:
        filters=filters.permute(2,0,1,3,4,5).contiguous() # permute to avoid making contiguous inside loop(5,16,1,5,5,5)
    ...
    padding=filters.size(0)//2  # sample filtering, that's to say pad = k/2
    if use_half:
    ...
    data_padded = torch.cat((Z,data,Z),0)  # (20,1,1,16,16,16,16)
    ...
    output=output.permute(1,2,0,3,4,5).contiguous()  # (1(batch_size),16(c_out),16,16,16,16)
    return output
