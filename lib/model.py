...
class NeighConsensus(torch.nn.Module):
    def __init__(self, use_cuda=True, kernel_sizes=[3,3,3], channels=[10,10,1], symmetric_mode=True):
        ...
        for i in range(num_layers):
            ...
            # nn_modules.append(nn.Dropout(p=0.5))  # use the dropout to erase over-fitting
...
def MutualMatching(corr4d):
    ...
    # get max
    corr4d_B_max,_=torch.max(corr4d_B,dim=1,keepdim=True)  # (batch_size, 1, 16, 16) B中每个点在A中匹配到的最佳位置及最大score
    corr4d_A_max,_=torch.max(corr4d_A,dim=3,keepdim=True)  # (batch_size, 1, 16, 16) A中每个点在B中匹配到的最佳位置及最大score

    eps = 1e-5
    corr4d_B=corr4d_B/(corr4d_B_max+eps)  # 如论文公式(5)所示  # [batch_idx,256,16,16]
    corr4d_A=corr4d_A/(corr4d_A_max+eps)  # 如论文公式(5)所示  # [batch_idx,16,16,256]

    corr4d_B=corr4d_B.view(batch_size, 1, fs1, fs2, fs3, fs4)  # [batch_idx, 1, 16, 16 ,16, 16]
    corr4d_A=corr4d_A.view(batch_size, 1, fs1, fs2, fs3, fs4)  # [batch_idx, 1, 16, 16 ,16, 16]

    corr4d=corr4d*(corr4d_A*corr4d_B)  # 双向权重系数处理，弱化非显著性正匹配的权重，如公式(4)
...
class ImMatchNet(nn.Module):
    ...
    def colorSpaceShare(self, Fa, Fb):  # Fa:(1,h,w,C)
        fa, fb = Fa.data.cpu().numpy(), Fb.data.cpu().numpy()
        eps = 0.001
        meanA = np.mean(np.mean(fa, axis=1), axis=1) + eps
        meanB = np.mean(np.mean(fb, axis=1), axis=1) + eps
        stdA = np.std(np.std(fa, axis=1), axis=1)
        stdB = np.std(np.std(fb, axis=1), axis=1)
        stdA, stdB = np.where(stdA == 0, 1, stdA), np.where(stdB == 0, 1, stdB)
        meanAB, stdAB = (meanA + meanB) / 2, (stdA + stdB) / 2

        Ca = np.zeros(fa.shape)
        for i in range(fa.shape[0]):
            Ca[i] = ((fa[i] - meanA[i]) / stdA[i]) * stdAB[i] + meanAB[i]
        return Variable(torch.from_numpy(Ca).type(torch.cuda.FloatTensor))
    ...
    def forward(self, tnf_batch): 
        ...
        feature_B = feature_B.permute(0, 2, 3, 1).contiguous()
        import copy
        Fa, Fb = copy.copy(feature_A), copy.copy(feature_B)
        feature_A = self.colorSpaceShare(Fa, Fb).permute(0, 3, 1, 2).contiguous()
        feature_B = self.colorSpaceShare(Fb, Fa).permute(0, 3, 1, 2).contiguous()
        ...
        corr4d = MutualMatching(corr4d)  # soft mutual nearest neighbour filtering
